...
export default class AddAxis {
  ...
  constructor(svg, opt) {
    ...
    // 创建X轴g元素
    const { itemStyle, yAxis, height } = this.config // 宽、高
    ...
    // X轴线    
    this.axisXLine = this.axisYGroup.append('path')  
    // 是否创建y轴g元素
    if(show){
      // Y轴g元素
  ...
  renderYAxis(data) {
    const self = this
    const { yHeight, itemStyle } = self.config // 宽、高
    const { bottom } = itemStyle.margin
 
    // x轴比例尺
    self.yScale = d3.scale.pow()  
      .domain([0, data.length - 1])  
      .range([yHeight - bottom, bottom])  
 
    // 添加x轴g元素
    let axisG = this.axisYGroup
    // X轴线
    ...
    // 处理update部分  
    update.call(::self.setYAxisTextAttribute) 
    // 获取并处理exit部分    
    update.exit().remove()  
    // 返回x轴比例尺
    return self.yScale
  ...
  renderXAxis(data) {
    const self = this
    // 获取一系列配置项
    const { xWidth, yHeight, itemStyle } = this.config
    const { ticks, show, pow } = this.config.yAxis
    const { bottom } = itemStyle.margin
    ...
    if(show){
      ...
      // 添加y轴g元素
      this.axisXGroup
        .call(::self.setXAxisAttribute, axis)
    }
    ...
    if(this.isYGridLine && show){
      ...
      this.gridYLine.selectAll('.domain').remove()  
