...
export default class AddAxis {
  ...
  setGroupAttribute(g) {
    g.attr('transform', (d, i) => `translate(${this.xScale(i)}, 0)`)
  }

  setUsetAttrubite(rect) {
    rect.attr('xlink:href', `#${this.useId}`)
  }
  ...
  renderYAxis(data) {
    ...
    const { ticks, pow, show } = this.config.yAxis
   ...
   */
  addElement(defs) {
    const { xItemStyle } = this.config
    defs.append('rect')
      .attr('fill', xItemStyle.fill)
      .attr('width', xItemStyle.width)
      .attr('height', xItemStyle.height)
      .attr('x', - xItemStyle.width / 2)
      .attr('y', 5)
      .attr('id', this.useId)
  }
}
