...
import { genSVGDocID, isNoData, interpolate, 
  countScale, getMousePosition } from '../../util/util'
import AddAxis from './addAxis'
import { showTips, hideTips } from './tips.js'

export default class PolygonBar {
  /**
   * 柱状图默认配置项
   * @return {object} 默认配置项
   */
  ...
  constructor(selector, opt) {
    ...
    itemStyle.scale = 200 / itemStyle.zoom
    // 创建svg元素
  ...
  renderData(data) {
    ...
    // 添加数据 
    enter.append('g').classed('group-data', true)
    // 添加顶部文字  
    enter.append('text')
    /* 
     * 处理update部分   
  ...
  setGroupAttribute(g) {
    ...
    g.attr('class', (d, i) => `group-${self.gId} group-${self.gId}-${i}`)
      .attr('transform', (d, i) => `translate(${transX + self.xScale(i)}, ${height - bottom})`)
  ...
  setTopTextAttribute(text) {
    const self = this
    const { topText, dur, itemStyle } = self.config
    const { scale } = itemStyle
    text.attr('class', 'top-text')
      ...
      .transition()
      .duration(dur)
      .attr('y', (d, i) => -self.rangeArr[i] * scale + i - 10)
  }
  ...
  addElement(defs) {
    ...
    const { stroke, strokeWidth, zoom } = itemStyle
    // 六边形的六个坐标点
    let points = '130,20, 20,40, 20,220, 130,200, 220,220, 220,40'
    points = points.split(',')
    let zoomPoints = []
