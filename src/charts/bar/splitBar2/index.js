...
export default class SplitBar {
  ...
  render(data) {
    ...
    // 最多画多少个小矩形
    let shapeMax = Math.floor(yHeight / (itemStyle.width + itemStyle.spacing)) 
    // 画多边形的个数比例尺
  ...
  renderData(data) {
    ...
    // 添加顶部文字  
    enter.append('text').classed('top-text', true)
  ...
  setTopTextAttribute(text) {
    ...
    text.attr('class', 'top-text')
      ...
      .transition()
      .duration(dur)
      .attr('y', (d, i) => -self.rangeArr[i] * (width + spacing) + i - 10)
  }
