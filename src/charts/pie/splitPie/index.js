/**
 * @Author:      zhanghq
 * @DateTime:    2017-09-17 20:44:41
 * @Description: 饼图
 * @Last Modified By:   zhanghq
 * @Last Modified Time:    2017-09-17 20:44:41
 */
...
export default class SplitPie {
  ...
  /**
   * Creates an instance of splitPie
  ...
  render (data) {
    ...
    // 获取并处理update部分
    let update = pathGroup.selectAll('g')
     ...
     */
    // 组元素update部分 
    update.call(::self.setGAttribute, arc, sum)   
     // 选择path元素   
    update.select('path')
   ...
   *  @return   {void}
   */
  setGAttribute(g, arc, sum) {
    const { dur } = this.config
    g.attr('class', (d, i) => `path-g-${i}` )
  ...
  */
  setPathAttribute (path, nums) {
    ...
    path.attr('fill', (d, i) => {
      ...
      .attr('d', 'M15.851,35.997 L11.531,38.877 L0.011,14.397 L20.171,-0.003' 
        + 'L23.051,4.317 L7.211,15.837 L15.851,35.997 Z')
  }
  ...
  setPolylineAttribute(polyline, points) {
    const { lineIndex, dur, width, height} = this.config
    polyline
      .attr('fill', 'none')
  ...
  getColor(idx) {
    ...
    let palette = _.merge([], defauleColor, this.config.color)
